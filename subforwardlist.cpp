...
{
    subforwardlist *newElem = new subforwardlist;
    newElem->data = d;
    newElem->next = NULL;
...
int pop_forward(subforwardlist **sfl) 	//удаление элемента из начала недосписка, если пустой - возвращать 0
{
    ...
    subforwardlist *firstElem = *sfl;
    subforwardlist *secondElem = firstElem->next;

    int popValue = firstElem->data;
    delete firstElem;
...
int erase_where(subforwardlist **sfl, unsigned int where)	//удаление элемента с порядковым номером where
{
    ...
    subforwardlist * eraseElem = prevElem->next;
    int popValue = eraseElem->data;
    prevElem->next = eraseElem->next;
    delete eraseElem;
    return popValue;
}
void clear(subforwardlist  **sfl)	//очистить содержимое недосписка
{
    subforwardlist  *topElem = *sfl;
    while(topElem != NULL)
    {
        subforwardlist  *oldTopElem = topElem;
